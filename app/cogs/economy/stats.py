...
class Stats(commands.Cog):
    ...
    async def stats(self, interaction: discord.Interaction) -> None:
        ...
        # Dice
        dice = user.get("dice", {})
        dc_wins = dice.get("wins", 0)
        dc_losses = dice.get("losses", 0)
        dc_ratio = f"{dc_wins}/{dc_losses}" if (dc_wins + dc_losses) > 0 else "N/A"
        embed.add_field(name="Dice W/L", value=dc_ratio, inline=True)
